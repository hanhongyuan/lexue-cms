	...
	<body>
		<div class="admin-main">
			...
			<blockquote class="layui-elem-quote" style="border-left:0px solid #009688;">
				<@common.permission per="sys:adres:add">
		...
		<script>
			...
			layui.use(['paging', 'upload', 'form'], function() {
                ...
                paging.init({
                    openWait: true,
                    url: '/adRes/data?v=' + new Date().getTime(), //地址
					elem: '#content', //内容容器
					params: { //发送到服务端的参数
					},
					type: 'POST',
					...
					complate: function() { //完成的回调
						...
						$('#content').children('tr').each(function() {
							...
							$that.children('td:last-child').children('a[data-opt=edit]').on('click', function() {
                                ...
                                $.get('/adRes/add', {id:$(this).data('id')}, function(form) {
                                    addBoxIndex = layer.open({
                                        ...
                                        success: function(layero, index) {
                                            ...
                                            layui.upload({
                                                url: '/adRes/upload',
                                                ext: 'mp4|jpg|png|gif|bmp|jpeg',
                                                title:'上传文件',
                                                size: 2048,
                                                elem: '.layui-upload-file', //指定原始元素，默认直接查找class="layui-upload-file"
                                                method: 'post' ,//上传接口的http类型
                                                before:function (data) {
                                                    loading=layer.msg('上传中...', {icon: 16,shade: [0.5, '#f5f5f5'],scrollbar: false,offset: '0px', time:10000000}) ;
                                                },
                                                success: function(res){
                                                    ...
                                                    if(res.code=200){
                                                        layer.msg("上传成功");
                                                        $("input[name='resourceURL']").val(res.message);
				...
				$('#add').on('click', function() {
					...
					$.get('/adRes/add', null, function(form) {
						addBoxIndex = layer.open({
							type: 1,
							title: '添加表单',
							content: form,
							...
							success: function(layero, index) {
                                ...
                                form.verify({
									url:[/^(http|https)\:\/\/([a-zA-Z0-9\.\-]+(\:[a-zA-Z0-9\.&amp;%\$\-]+)*@)?((25[0-5]|2[0-4][0-9]|[0-1]{1}[0-9]{2}|[1-9]{1}[0-9]{1}|[1-9])\.(25[0-5]|2[0-4][0-9]|[0-1]{1}[0-9]{2}|[1-9]{1}[0-9]{1}|[1-9]|0)\.(25[0-5]|2[0-4][0-9]|[0-1]{1}[0-9]{2}|[1-9]{1}[0-9]{1}|[1-9]|0)\.(25[0-5]|2[0-4][0-9]|[0-1]{1}[0-9]{2}|[1-9]{1}[0-9]{1}|[0-9])|([a-zA-Z0-9\-]+\.)*[a-zA-Z0-9\-]+\.[a-zA-Z]{2,4})(\:[0-9]+)?(\/[^/][a-zA-Z0-9\.\,\?\'\\/\+&amp;%\$#\=~_\-@]*)*$/,'网址格式不正确']
								});
								form.on('submit(edit)', function(data) {
									$.ajax({
										url:'/adRes/save',
