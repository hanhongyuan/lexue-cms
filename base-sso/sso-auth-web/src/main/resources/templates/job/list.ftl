	...
	<body>
		...
		<script type="text/html" id="tpl">
			...
			<tr>
				...
				<td>
					<@common.permission per="sys:job:start">
                        <a href="javascript:layui.startJob('{{ item.id }}');" data-id="{{ item.id }}" data-opt="open" class="layui-btn layui-btn-mini"><i class="layui-icon">&#xe652;</i>开始</a>
					</@common.permission>
					<@common.permission per="sys:job:stop">
                        <a href="javascript:layui.stopJob('{{ item.id }}');" data-id="{{ item.id }}" data-opt="stop" class="layui-btn layui-btn-mini"><i class="layui-icon">&#xe651;</i>暂停</a>
					</@common.permission>
					<@common.permission per="sys:job:delete">
		...
		<script>
			...
			layui.use(['paging', 'form'], function() {
                ...
                paging.init({
                    openWait: true,
                    url: '/job/data?v=' + new Date().getTime(), //地址
					elem: '#content', //内容容器
					params: { //发送到服务端的参数
					},
					type: 'POST',
					...
					complate: function() { //完成的回调
						...
						$('#content').children('tr').each(function() {
							...
							$that.children('td:last-child').children('a[data-opt=edit]').on('click', function() {
                                ...
                                $.get('/job/add', {id:$(this).data('id')}, function(form) {
				...
				$('#add').on('click', function() {
					...
					$.get('/job/add', null, function(form) {
						addBoxIndex = layer.open({
							type: 1,
							title: '添加表单',
							content: form,
