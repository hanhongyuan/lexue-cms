	...
	<body>
		...
		<script type="text/html" id="tpl">
			...
			<tr>
                ...
                <td>{{ item.h5url || '' }}</td>
				<td><img width="100px" height="50px" src="{{ item.backgroundImage || '' }}" /></td>
				<td>
		...
		<script>
			...
			layui.use(['paging', 'upload','form'], function() {
                ...
                paging.init({
                    openWait: true,
                    url: '/wx/GroupData?v=' + new Date().getTime(), //地址
					elem: '#content', //内容容器
					params: { //发送到服务端的参数
					},
					type: 'POST',
					...
					complate: function() { //完成的回调
						...
						$('#content').children('tr').each(function() {
							...
							$that.children('td:last-child').children('a[data-opt=edit]').on('click', function() {
                                ...
                                $.get('/wx/addG', {id:$(this).data('id')}, function(form) {
                                        ...
                                        success: function(layero, index) {
                                            ...
                                            layui.upload({
                                                ...
                                                success: function(res){
                                                    ...
                                                    if(res.code=200){
														layer.msg("上传成功");
														$("input[name='backgroundImage']").val(res.message);
													}
                                                }
                                            });
				...
				$('#add').on('click', function() {
					...
					$.get('/wx/addG', null, function(form) {
						addBoxIndex = layer.open({
							type: 1,
							title: '添加角色',
							content: form,
							...
							success: function(layero, index) {
                                ...
                                form.verify({
                                    backgroundImage:function (value,item) {
                                        if(value==""||value==''){
                                            return "背景图片不能为空";
                                        }
                                    }
